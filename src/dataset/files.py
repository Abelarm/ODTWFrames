...
class ImageMatrix:
    ...
    def __init__(self, ref_pattern, time_series, class_num, rho, starting_path, ref_id=None):
        ...
        if time_series_prop['set'] not in ['train', 'validation'] and self.from_pattern_idx is None:
            ...
            if 'gunpoint' in starting_path:
                path = f'{self.matrix_name}Mat-{time_series_prop["set"]}_rho-{rho}_ref-id-*_stream-id-{time_series_prop["id"]}.npy'
            else:
                path = f'{self.matrix_name}Mat-{time_series_prop["set"]}_length-{time_series_prop["length"]}_' \
        ...
        if 'gunpoint' in starting_path:
            self.path = f'{self.matrix_name}Mat-{time_series_prop["set"]}_rho-{rho}' \
                        f'_ref-id-{selected}_stream-id-{time_series_prop["id"]}.npy'
        else:
            self.path = f'{self.matrix_name}Mat-{time_series_prop["set"]}_length-{time_series_prop["length"]}_' \
                   f'noise-{time_series_prop["noise"]}_warp-{time_series_prop["warp"]}' \
                   f'_shift-{time_series_prop["shift"]}_outliers-{time_series_prop["outliers"]}' \
                   f'_rho-{rho}_ref-id-{selected}_stream-id-{time_series_prop["id"]}.npy'

        self.img = np.load(join(starting_path, self.path))
    ...
    def images(self, window_size):
        ...
        images = []
        labels = []
        for i in range(self.starting_skip, self.img.shape[1] - window_size):
            tmp_img = self.img[:, i:i + window_size]
...
class DTW(ImageMatrix):
    matrix_name = 'dtw'


class RP(ImageMatrix):
    matrix_name = 'dtw'

    def __init__(self, ref_pattern, time_series, class_num, starting_path, rho, ref_id=None, **kwargs):
        self.ref_pattern = ref_pattern
        self.time_series = time_series
        self.class_num = class_num
        self.starting_skip = 0
        ...
        if time_series_prop['set'] not in ['train', 'validation'] and self.from_pattern_idx is None:
            ...
            if 'gunpoint' in starting_path:
                path = f'rpMat-{time_series_prop["set"]}_ref-id-*_stream-id-{time_series_prop["id"]}.npy'
            else:
                path = f'rpMat-{time_series_prop["set"]}_length-{time_series_prop["length"]}_' \
                       f'noise-{time_series_prop["noise"]}_warp-{time_series_prop["warp"]}' \
                       f'_shift-{time_series_prop["shift"]}_outliers-{time_series_prop["outliers"]}' \
                       f'_ref-id-*_stream-id-{time_series_prop["id"]}.npy'
        ...
        if 'gunpoint' in starting_path:
            self.path = f'rpMat-{time_series_prop["set"]}' \
                        f'_ref-id-{selected}_stream-id-{time_series_prop["id"]}.npy'
        else:
            self.path = f'rpMat-{time_series_prop["set"]}_length-{time_series_prop["length"]}_' \
                   f'noise-{time_series_prop["noise"]}_warp-{time_series_prop["warp"]}' \
                   f'_shift-{time_series_prop["shift"]}_outliers-{time_series_prop["outliers"]}' \
                   f'_ref-id-{selected}_stream-id-{time_series_prop["id"]}.npy'
