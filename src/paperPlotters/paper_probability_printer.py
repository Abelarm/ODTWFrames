...
rho_arr = ['0.100', '0.100_base_A', '0.100_base_B', '0.100_base_AB']
num_sample = 7
...
full_pro = np.zeros((len(rho_arr), total_len, y_dim))
...
gs = fig.add_gridspec(len(rho_arr) + 1, (len(rho_arr) + 1) * 2,
...
for idx, rho in enumerate(rho_arr):
    ...
    if rho != rho_arr[-1]:
        ...
        f_axi.axes.set_ylabel(r'Time')  # <<<<<<<< ADD THIS
