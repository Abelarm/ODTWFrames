...
def get_model(x_dim, y_dim):
    ...
    conv_x = Conv1D(filters=n_feature_maps, kernel_size=5, padding='same')(input_layer)
    ...
    conv_x = Conv1D(filters=n_feature_maps * 2, kernel_size=5, padding='same')(output_block_1)
    conv_x = BatchNormalization()(conv_x)
    conv_x = Activation('relu')(conv_x)

    conv_y = Conv1D(filters=n_feature_maps * 2, kernel_size=3, padding='same')(conv_x)
    conv_y = BatchNormalization()(conv_y)
    conv_y = Activation('relu')(conv_y)

    conv_z = Conv1D(filters=n_feature_maps * 2, kernel_size=1, padding='same')(conv_y)
    conv_z = BatchNormalization()(conv_z)
    ...
    conv_x = Conv1D(filters=n_feature_maps * 2, kernel_size=5, padding='same')(output_block_2)
    conv_x = BatchNormalization()(conv_x)
    conv_x = Activation('relu')(conv_x)
