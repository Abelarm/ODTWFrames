...
def plot_confusion_matrix(y_true, y_pred, class_names, save_dir):
    ...
    for title, normalize in titles_options:
        fig = plt.figure(dpi=dpi)
        conf = confusion_matrix(y_true, y_pred, normalize=normalize, labels=[int(i) for i in class_names])

        con_mat_df = pd.DataFrame(conf, index=class_names, columns=class_names)
        sns.heatmap(con_mat_df, annot=True, cmap=plt.cm.Blues, annot_kws={"size": 15 / np.sqrt(len(con_mat_df))})
